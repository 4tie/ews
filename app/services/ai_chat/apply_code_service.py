...
async def create_run_scoped_candidate(
    *,
    ...
    """Validate the legacy AI chat request and delegate to the canonical backtest proposal path."""
    ...
    candidate_mode = ProposalCandidateMode.CODE_PATCH if isinstance(code, str) and code.strip() else ProposalCandidateMode.PARAMETER_ONLY
    ...
    return await runtime.create_backtest_run_proposal_candidate(run.run_id, payload)
...
async def rerun_candidate_backtest(
    *,
    ...
    response = await runtime.run_backtest(payload)
    return {
...
async def compare_candidate_vs_baseline(
    *,
    ...
    comparison = await runtime.compare_backtest_runs(baseline_run_id, candidate_run_id)
    return {
        "baseline_run_id": baseline_run_id,
